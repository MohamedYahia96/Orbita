...
GITHUB_WEBHOOK_SECRET = <long-random-secret-for-github-webhook-signature>
```

> ⚠️ **تنبيه:** لا تشارك `VAPID_PRIVATE_KEY` أو `CRON_SECRET` أو `GITHUB_WEBHOOK_SECRET` مع أي أحد ولا ترفعهم على GitHub.
...
5. أضف **Webhook Secret** (إلزامي للإنتاج) نفس قيمة `GITHUB_WEBHOOK_SECRET`.
