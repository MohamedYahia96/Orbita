...
export default function SmartDigestPage() {
    ...
    const renderSection = (title: string, items: DigestItem[]) => {
        ...
        return (
            <div className="mb-10">
                <h2 className="text-xl font-bold mb-4 flex items-center gap-2 border-b border-(--color-border) pb-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {items.map(item => (
                        <Card key={item.id} className="p-4 hover:border-accent transition-colors">
                            {item.image && (
                                          <div className="relative w-full h-32 rounded-lg bg-(--color-bg-tertiary) mb-3 overflow-hidden">
                            ...
                            <h3 className="font-semibold text-(--color-text-primary) mb-2 line-clamp-2 leading-tight">
                                {item.title}
                            </h3>
                            {item.content && (
                                <p className="text-sm opacity-60 line-clamp-2">
                                    {item.content.replace(/<[^>]+>/g, '')}
                                     ...
                                     <div className="mt-4 pt-4 border-t border-(--color-border) flex justify-between items-center">
                                ...
                                <div className="flex gap-3">
                                   <button 
                                      onClick={() => handleAddNote(item.id, item.note)}
                                                  className="text-(--color-text-primary) opacity-60 hover:opacity-100 flex items-center gap-1 text-sm font-medium bg-transparent border-none cursor-pointer"
                                   >
